...
import com.skyblock.dungeon.listener.DungeonNaturalSpawnGuard;
...
public class SkyblockPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if (dungeonWorld == null) {
            ...
            // The old DungeonEntityVisibilityCuller (client-side mob
            // hide/show based on range + line-of-sight) has been removed
            // entirely per report: it was making mobs standing right next
            // to a player invisible while making distant mobs flood the
            // screen - its near/far logic was backwards in practice, and
            // the spawn-rate revert below removes most of the reason it
            // existed in the first place (far fewer ambient mobs alive
            // per room now, so per-player render cost is back to normal
            // without any culling at all).
            //
            // Instead, this guard stops mobs from being a problem at the
            // source: it cancels vanilla natural spawning everywhere in
            // the dungeon world (the actual cause of mobs appearing in
            // already-cleared/already-generated areas on an ongoing
            // basis, and of unleveled "wild" mobs mixed in with the
            // intentional roster) and guarantees nothing hostile can
            // exist on Floor 0, the entrance hub.
            DungeonNaturalSpawnGuard dungeonNaturalSpawnGuard =
                new DungeonNaturalSpawnGuard(dungeonFloorManager, dungeonFloorBounds, dungeonMobLevelApplicator);
